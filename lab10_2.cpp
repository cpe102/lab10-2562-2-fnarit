...
int calScore(int x,int y,int z){
	...
	//Return the score. 
	score = cardScores[x]+cardScores[y];
	return score%10;
}
...
int main(){	
	srand(time(0));
	int playerScore, yugiScore, playerAction, yugiAction,num;
	...
	if(playerAction == 1){
		...
		cout << "Your 3rd card: " << cardNames[playerCards[0]] << "\n";
		num = calScore(playerCards[0],playerCards[2],playerCards[2]);
		playerScore = (playerScore + num)%10;
	...
	if(yugiAction == 1){
		...
		cout << "Yugi's 3rd card: " << cardNames[yugiCards[0]] << "\n";
		num = calScore(yugiCards[0],yugiCards[2],yugiCards[2]);
		yugiScore = (yugiScore + num)%10;
